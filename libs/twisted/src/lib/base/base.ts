...
import { BaseApiGames, BaseConstants } from "./base.const"
...
export class BaseApi<Region extends string> {
    ...
    private setParams(param: IBaseApiParams): void {
    ...
    private getApiUrl(endpoint: IEndpoint, params: IParams): string {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    private isServiceUnavailableError(e: any): boolean {
        if (!e) {
            return false
        }
    ...
    private async retryRateLimit<T>(
        ...
        for (let i = 0; i < this.rateLimitRetryAttempts; i++) {
            try {
                return await this.request<T>(region, endpoint, params, forceError)
            } catch (error) {
